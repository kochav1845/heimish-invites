...
import AIImageOptions from './AIImageOptions';
...
const IntegrationsPage: React.FC = () => {
  ...
  const [aiOption, setAIOption] = useState<'chat' | 'image' | 'video' | 'generate-image' | null>(null);
  const [imageGenOption, setImageGenOption] = useState<'text-to-image' | 'image-to-image' | null>(null);
  const [videoOption, setVideoOption] = useState<'image-to-video' | 'extend-video' | 'modify-video' | null>(null);
  const [resultGenerationId, setResultGenerationId] = useState<string | null>(null);
  const [savedVideos, setSavedVideos] = useState<{url: string, generationId: string, type?: string}[]>([]);
  const [aiVideos, setAiVideos] = useState<{url: string, prompt?: string}[]>([]);
  const [aiImages, setAiImages] = useState<{url: string, prompt?: string}[]>([]);
  ...
  useEffect(() => {
    ...
    if (hash && hash !== 'invitations') {
      ...
      setSelectedCategory('invitations');
    }
  ...
  const handleAIOptionSelect = (option: 'chat' | 'image' | 'video') => {
    setAIOption(option);
  };

  const handleImageGenOptionSelect = (option: 'text-to-image' | 'image-to-image') => {
    setImageGenOption(option);
  };

  const handleVideoOptionSelect = (option: 'image-to-video' | 'extend-video' | 'modify-video') => {
  ...
  const renderAIContent = () => {
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
              <motion.button
                ...
                onClick={() => handleAIOptionSelect('chat')}
              ...
              <motion.button
                ...
                onClick={() => handleAIOptionSelect('generate-image')}
                className="group relative bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-sm p-8 rounded-xl overflow-hidden hover:scale-105 transition-transform duration-300"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-12 h-12 mb-4 text-purple-400 group-hover:text-purple-300 transition-colors">
                ...
                <h3 className="text-xl font-michroma text-white mb-2">Generate Image</h3>
                <p className="text-gray-400 group-hover:text-gray-300">
                  Create AI-generated images from text or other images
                </p>
              </motion.button>
              ...
              <motion.button
                ...
                onClick={() => handleAIOptionSelect('image')}
                className="group relative bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-sm p-8 rounded-xl overflow-hidden hover:scale-105 transition-transform duration-300"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-12 h-12 mb-4 text-purple-400 group-hover:text-purple-300 transition-colors">
              ...
              <motion.button
                ...
                onClick={() => handleAIOptionSelect('video')}
                className="group relative bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-sm p-8 rounded-xl overflow-hidden hover:scale-105 transition-transform duration-300"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-purple-500/20 to-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-12 h-12 mb-4 text-purple-400 group-hover:text-purple-300 transition-colors">
            ...
            {aiImages.length > 0 && (
              <div className="mt-16">
                <h3 className="text-2xl font-michroma text-white mb-8 text-center">
                  AI-Generated Images
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {aiImages.map((image, index) => (
                    ...
                    >
                      ...
                      <div className="p-4">
                        <h4 className="text-white font-medium mb-2">AI-Generated Image {index + 1}</h4>
                        {image.prompt && (
    ...
    switch (aiOption) {
      ...
      case 'generate-image':
        if (!imageGenOption) {
          return <AIImageOptions onSelect={handleImageGenOptionSelect} />;
        }
        return (
          <ImageCreator
            type={imageGenOption}
            onBack={() => setImageGenOption(null)}
            onImageSaved={(url, generationId) => {
              // Add to AI images if we're in the AI section
              if (selectedCategory === 'ai') {
                setAiImages(prev => {
                  if (!prev.some(v => v.url === url)) {
                    return [...prev, { url, prompt: prompt }];
                  }
                  return prev;
                });
              }
      ...
      case 'video':
        return (
          <VideoCreator
            type={aiOption === 'image' ? 'image-to-video' : 'modify-video'}
            onBack={() => setAIOption(null)}
          />
        );
      default:
        return null;
  ...
  const renderVideoContent = () => {
    if (!videoOption) {
      return <VideoOptions onSelect={handleVideoOptionSelect} />;
    }
    
    return (
      <VideoCreator
        type={videoOption}
        onBack={() => setVideoOption(null)}
        onVideoSaved={(url, generationId) => {
          if (url && generationId) {
            setSavedVideos(prev => {
              if (!prev.some(v => v.url === url)) {
                return [...prev, { url, generationId }];
              }
              return prev;
            });
            
            // Also add to AI videos if we're in the AI section
            if (selectedCategory === 'ai') {
              setAiVideos(prev => {
                if (!prev.some(v => v.url === url)) {
                  return [...prev, { url, prompt: '' }];
                }
                return prev;
              });
            }
